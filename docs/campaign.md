...
Request.ListCampaigns request = new Request.ListCampaigns(
    ...
    AvailableFrom = "2023-06-26T06:42:42.000000Z",  // 指定された日時以降に適用可能期間が含まれているか
    AvailableTo = "2026-07-05T04:44:22.000000Z",  // 指定された日時以前に適用可能期間が含まれているか
    Page = 1,  // ページ番号
...
Request.CreateCampaign request = new Request.CreateCampaign(
    "rFxY0nh8lQ2iQavwvhDr8TNB4vIcRTpSaCV5lZtxsN8hQh23jWL68GyttBaIaA6bT2oimSP8aDw1fwYQo1a8Jvio1NlXmWokT3fCZ0aqdulZZGglvs1mmHvcGJdXuMvjofsG8E4KIFxs3y0EBuTM1S0iPJraQIMtAPJ1JN9CtWW30Uo4UAg9arJ4XCMrwN15cI", // キャンペーン名
    "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", // マネーID
    "2023-06-03T21:51:52.000000Z", // キャンペーン開始日時
    "2021-05-14T13:06:38.000000Z", // キャンペーン終了日時
    5088, // キャンペーンの適用優先度
    "topup" // イベント種別
) {
    ...
    PointExpiresAt = "2022-12-24T13:41:12.000000Z",  // ポイント有効期限(絶対日時指定)
    PointExpiresInDays = 1673,  // ポイント有効期限(相対日数指定)
    ...
    ApplicableDaysOfWeek = new int[]{4, 4},  // キャンペーンを適用する曜日 (複数指定)
    ...
    ApplicableShopLabelIds = new string[]{"xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"},  // キャンペーン適用対象となる店舗ラベルIDのリスト
    BlacklistedShopIds = new string[]{"xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"},  // キャンペーン適用対象外となる店舗IDのリスト(ブラックリスト方式)
    MinimumNumberOfProducts = 3092,  // キャンペーンを適用する1会計内の商品個数の下限
    MinimumNumberOfAmount = 7800,  // キャンペーンを適用する1会計内の商品総額の下限
    MinimumNumberForCombinationPurchase = 1202,  // 複数種類の商品を同時購入するときの商品種別数の下限
    ExistInEachProductGroups = false,  // 複数の商品グループにつき1種類以上の商品購入によって発火するキャンペーンの指定フラグ
    MaxPointAmount = 4574,  // キャンペーンによって付与されるポイントの上限
    MaxTotalPointAmount = 8794,  // キャンペーンによって付与されるの1人当たりの累計ポイントの上限
    ...
    BudgetCapsAmount = 1156858888,  // キャンペーン予算上限
};
...
Request.UpdateCampaign request = new Request.UpdateCampaign(
    ...
    StartsAt = "2023-04-19T18:59:26.000000Z",  // キャンペーン開始日時
    EndsAt = "2021-02-18T16:17:07.000000Z",  // キャンペーン終了日時
    ...
    PointExpiresAt = "2025-01-23T05:24:22.000000Z",  // ポイント有効期限(絶対日時指定)
    PointExpiresInDays = 5079,  // ポイント有効期限(相対日数指定)
    IsExclusive = true,  // キャンペーンの重複設定
    Subject = "money",  // ポイント付与の対象金額の種別
    AmountBasedPointRules = new object[]{new Dictionary<string, object>(){{"point_amount",5}, {"point_amount_unit","percent"}, {"subject_more_than_or_equal",1000}, {"subject_less_than",5000}}},  // 取引金額ベースのポイント付与ルール
    ...
    ApplicableShopLabelIds = new string[]{"xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"},  // キャンペーン適用対象となる店舗ラベルIDのリスト
    ...
    MinimumNumberOfProducts = 9473,  // キャンペーンを適用する1会計内の商品個数の下限
    MinimumNumberOfAmount = 5237,  // キャンペーンを適用する1会計内の商品総額の下限
    MinimumNumberForCombinationPurchase = 6719,  // 複数種類の商品を同時購入するときの商品種別数の下限
    ExistInEachProductGroups = false,  // 複数の商品グループにつき1種類以上の商品購入によって発火するキャンペーンの指定フラグ
    MaxPointAmount = 6906,  // キャンペーンによって付与されるポイントの上限
    MaxTotalPointAmount = 4708,  // キャンペーンによって付与されるの1人当たりの累計ポイントの上限
    ApplicableAccountMetadata = new Dictionary<string, object>(){{"key","sex"}, {"value","male"}},  // ウォレットに紐付くメタデータが特定の値を持つときにのみ発火するキャンペーンを登録します。
    ApplicableTransactionMetadata = new Dictionary<string, object>(){{"key","rank"}, {"value","bronze"}},  // 取引時に指定するメタデータが特定の値を持つときにのみ発火するキャンペーンを登録します。
    BudgetCapsAmount = 276815970,  // キャンペーン予算上限
};
Response.Campaign response = await request.Send(client);
```
