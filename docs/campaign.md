...
Request.ListCampaigns request = new Request.ListCampaigns(
    ...
    AvailableFrom = "2025-10-10T09:49:31.000000Z",  // 指定された日時以降に適用可能期間が含まれているか
    AvailableTo = "2020-08-22T00:32:28.000000Z",  // 指定された日時以前に適用可能期間が含まれているか
    Page = 1,  // ページ番号
...
Request.CreateCampaign request = new Request.CreateCampaign(
    "VzvPSXDUkbgX2oBshUtXGZ9lfp9TwgYPOmismihXWyqdhqoMR6oAdT5yPsPRTmUYdZdYDDGZDuZn0XgqQIqTu14tSh13qLZDYdRTWbMgZiB4q5yXIKvcyeytZUeCOzn479", // キャンペーン名
    "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", // マネーID
    "2025-06-03T12:25:53.000000Z", // キャンペーン開始日時
    "2024-05-12T16:35:49.000000Z", // キャンペーン終了日時
    1672, // キャンペーンの適用優先度
    "external-transaction" // イベント種別
) {
    BearPointShopId = "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",  // ポイント負担先店舗ID
    Description = "CQ6mogsi4OQ6jQwMdVQzET3CTZR3naadmHoO937wRncWgLEMvwuXtyGneCNJhR9grzsET9HHziGJ2iqEYWh5QfKEnNvZa5",  // キャンペーンの説明文
    Status = "disabled",  // キャンペーン作成時の状態
    PointExpiresAt = "2021-06-02T12:40:34.000000Z",  // ポイント有効期限(絶対日時指定)
    PointExpiresInDays = 2615,  // ポイント有効期限(相対日数指定)
    ...
    ApplicableDaysOfWeek = new int[]{0, 6},  // キャンペーンを適用する曜日 (複数指定)
    ...
    MinimumNumberOfProducts = 3325,  // キャンペーンを適用する1会計内の商品個数の下限
    MinimumNumberOfAmount = 5450,  // キャンペーンを適用する1会計内の商品総額の下限
    MinimumNumberForCombinationPurchase = 1134,  // 複数種類の商品を同時購入するときの商品種別数の下限
    ExistInEachProductGroups = true,  // 複数の商品グループにつき1種類以上の商品購入によって発火するキャンペーンの指定フラグ
    MaxPointAmount = 9860,  // キャンペーンによって付与されるポイントの上限
    MaxTotalPointAmount = 8364,  // キャンペーンによって付与されるの1人当たりの累計ポイントの上限
    ...
    BudgetCapsAmount = 1945240891,  // キャンペーン予算上限
};
...
Request.UpdateCampaign request = new Request.UpdateCampaign(
    ...
    Name = "b7878ag0GpEoXRZP9Tuo6ihkLt",  // キャンペーン名
    StartsAt = "2021-06-11T15:52:46.000000Z",  // キャンペーン開始日時
    EndsAt = "2020-01-02T13:45:20.000000Z",  // キャンペーン終了日時
    Priority = 6509,  // キャンペーンの適用優先度
    Event = "topup",  // イベント種別
    Description = "VgJ",  // キャンペーンの説明文
    Status = "enabled",  // キャンペーン作成時の状態
    PointExpiresAt = "2022-02-06T15:10:21.000000Z",  // ポイント有効期限(絶対日時指定)
    PointExpiresInDays = 8069,  // ポイント有効期限(相対日数指定)
    ...
    MinimumNumberOfProducts = 420,  // キャンペーンを適用する1会計内の商品個数の下限
    MinimumNumberOfAmount = 1946,  // キャンペーンを適用する1会計内の商品総額の下限
    MinimumNumberForCombinationPurchase = 1142,  // 複数種類の商品を同時購入するときの商品種別数の下限
    ExistInEachProductGroups = false,  // 複数の商品グループにつき1種類以上の商品購入によって発火するキャンペーンの指定フラグ
    MaxPointAmount = 3745,  // キャンペーンによって付与されるポイントの上限
    MaxTotalPointAmount = 4663,  // キャンペーンによって付与されるの1人当たりの累計ポイントの上限
    ApplicableAccountMetadata = new Dictionary<string, object>(){{"key","sex"}, {"value","male"}},  // ウォレットに紐付くメタデータが特定の値を持つときにのみ発火するキャンペーンを登録します。
    ApplicableTransactionMetadata = new Dictionary<string, object>(){{"key","rank"}, {"value","bronze"}},  // 取引時に指定するメタデータが特定の値を持つときにのみ発火するキャンペーンを登録します。
    BudgetCapsAmount = 2113254599,  // キャンペーン予算上限
};
Response.Campaign response = await request.Send(client);
```
