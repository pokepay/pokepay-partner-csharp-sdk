...
Request.ListCampaigns request = new Request.ListCampaigns(
    ...
    AvailableFrom = "2020-10-24T00:30:31.000000Z",  // 指定された日時以降に適用可能期間が含まれているか
    AvailableTo = "2022-02-06T00:32:11.000000Z",  // 指定された日時以前に適用可能期間が含まれているか
    Page = 1,  // ページ番号
...
Request.CreateCampaign request = new Request.CreateCampaign(
    "cWgLEMvwuXtyGneCNJhR9grzsET9HHziGJ2iqEYWh5QfKEnNvZa51B6RuNHWw3kkEIImb7878ag0GpEoXRZP9Tuo6ihkLtNpmjVgJl2arbhJouxWQ6FlBm7k1iTzlm9ILQGKVJoUCSY35cdkgvsbAYCbaEHjTHUmx8bpMxYByLz0xsJRhRVsB9HjzBAZfWzO75yHWR5FLMa9CO3GmqQepv7doxpRjgZI2VSDvLJkkZMMdEA", // キャンペーン名
    "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", // マネーID
    "2023-04-10T19:26:30.000000Z", // キャンペーン開始日時
    "2021-09-11T00:32:00.000000Z", // キャンペーン終了日時
    1494, // キャンペーンの適用優先度
    "external-transaction" // イベント種別
) {
    BearPointShopId = "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",  // ポイント負担先店舗ID
    Description = "avAje3PJg4zkA5dwRQrAEDCEBzCTk0pNAGkxkj3y6QjLE9oTv9S3Zg4O5dK9OBTn3gY0HIwJr5Xn6R9PIw5eC52tvIBnMyMg4CnT2dj7ORUTt4jEgn4792da7QYy7V605lzcB",  // キャンペーンの説明文
    Status = "disabled",  // キャンペーン作成時の状態
    PointExpiresAt = "2022-04-28T19:09:49.000000Z",  // ポイント有効期限(絶対日時指定)
    PointExpiresInDays = 2747,  // ポイント有効期限(相対日数指定)
    ...
    ApplicableDaysOfWeek = new int[]{1, 6, 1},  // キャンペーンを適用する曜日 (複数指定)
    ...
    MinimumNumberOfProducts = 3946,  // キャンペーンを適用する1会計内の商品個数の下限
    MinimumNumberOfAmount = 743,  // キャンペーンを適用する1会計内の商品総額の下限
    MinimumNumberForCombinationPurchase = 1404,  // 複数種類の商品を同時購入するときの商品種別数の下限
    ExistInEachProductGroups = false,  // 複数の商品グループにつき1種類以上の商品購入によって発火するキャンペーンの指定フラグ
    MaxPointAmount = 4302,  // キャンペーンによって付与されるポイントの上限
    MaxTotalPointAmount = 4821,  // キャンペーンによって付与されるの1人当たりの累計ポイントの上限
    ...
    BudgetCapsAmount = 179278059,  // キャンペーン予算上限
};
...
Request.UpdateCampaign request = new Request.UpdateCampaign(
    ...
    Name = "K5UmBamQcUvvHD25XYGaGoRmlkWpVKSQYACWhdJgT5oXIAxp1c5Q2vG7By91KC2xkwbMvROWfUAhh6XnZz0yJYgRGAM6oTzljbZYS9b6qmrSFaDiVxdn1z0TuA7dLQ8GnuuGnm3um0ZKYlqHYAPfacx4ba4pxXiFCicQd3QQrdtpp5IlW8KnTaroT8w3801ZxeZpTa0FFkkUFLVCDKp9TvCsVFg3Dy6t9FVfvRBKOl2QQeBI5NM6J7Eh",  // キャンペーン名
    StartsAt = "2023-04-15T10:03:23.000000Z",  // キャンペーン開始日時
    EndsAt = "2024-01-05T02:21:02.000000Z",  // キャンペーン終了日時
    Priority = 2411,  // キャンペーンの適用優先度
    Event = "external-transaction",  // イベント種別
    Description = "yYle2ZOPXJOiEYcNwwBKhoxCdqw8SDS6L7O6ohLm8HBuYz7E9Zu",  // キャンペーンの説明文
    Status = "disabled",  // キャンペーン作成時の状態
    PointExpiresAt = "2024-11-23T05:44:34.000000Z",  // ポイント有効期限(絶対日時指定)
    PointExpiresInDays = 329,  // ポイント有効期限(相対日数指定)
    ...
    BlacklistedShopIds = new string[]{"xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx"},  // キャンペーン適用対象外となる店舗IDのリスト(ブラックリスト方式)
    MinimumNumberOfProducts = 1177,  // キャンペーンを適用する1会計内の商品個数の下限
    MinimumNumberOfAmount = 53,  // キャンペーンを適用する1会計内の商品総額の下限
    MinimumNumberForCombinationPurchase = 7508,  // 複数種類の商品を同時購入するときの商品種別数の下限
    ExistInEachProductGroups = true,  // 複数の商品グループにつき1種類以上の商品購入によって発火するキャンペーンの指定フラグ
    MaxPointAmount = 3863,  // キャンペーンによって付与されるポイントの上限
    MaxTotalPointAmount = 8362,  // キャンペーンによって付与されるの1人当たりの累計ポイントの上限
    ApplicableAccountMetadata = new Dictionary<string, object>(){{"key","sex"}, {"value","male"}},  // ウォレットに紐付くメタデータが特定の値を持つときにのみ発火するキャンペーンを登録します。
    ApplicableTransactionMetadata = new Dictionary<string, object>(){{"key","rank"}, {"value","bronze"}},  // 取引時に指定するメタデータが特定の値を持つときにのみ発火するキャンペーンを登録します。
    BudgetCapsAmount = 1727743105,  // キャンペーン予算上限
};
Response.Campaign response = await request.Send(client);
```
