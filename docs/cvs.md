...
Request.CreateCvsAuthorization request = new Request.CreateCvsAuthorization(
    ...
    ReceiverUserId = "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx",  // チャージ先エンドユーザーのID
    Description = "コンビニチャージ",  // 取引履歴に表示する説明文
    TopupQuotaId = 8113,  // チャージ可能枠ID
...
```csharp
Request.CancelCvsAuthorization request = new Request.CancelCvsAuthorization(
    "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx" // コンビニ決済の申し込みID
);
Response.CvsAuthorization response = await request.Send(client);
```
