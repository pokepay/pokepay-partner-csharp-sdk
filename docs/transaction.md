...
Request.GetCpmToken request = new Request.GetCpmToken(
    "l7qDDnWfDz83II3SsVbGfW" // CPMトークン
);
...
Request.CreateTransaction request = new Request.CreateTransaction(
    ...
    MoneyAmount = 8495,
    PointAmount = 9853,
    PointExpiresAt = "2026-02-02T11:05:13.000000Z",  // ポイント有効期限
...
Request.CreateTransactionGroup request = new Request.CreateTransactionGroup(
    "HFo8xzE1tgCZyMtC" // 作成するトランザクショングループの名称です。
);
...
Request.CreateTopupTransaction request = new Request.CreateTopupTransaction(
    ...
    MoneyAmount = 2088,  // マネー額
    PointAmount = 9291,  // ポイント額
    PointExpiresAt = "2024-10-11T03:24:21.000000Z",  // ポイント有効期限
...
Request.CreateCpmTransaction request = new Request.CreateCpmTransaction(
    "aCm6v4bOQPdSecOojChLhu", // CPMトークン
    "xxxxxxxx-xxxx-xxxx-xxxx-xxxxxxxxxxxx", // 店舗ID
    8773.0 // 取引金額
) {
...
Request.RefundTransaction request = new Request.RefundTransaction(
    ...
    ReturningPointExpiresAt = "2021-07-25T16:03:55.000000Z",  // 返却ポイントの有効期限
};
Response.TransactionDetail response = await request.Send(client);
```
